...
init_img_path = r"PATH_TO\porsche.png"
...
class StableDiffusionInference:
    def __init__(
        ...
        self.sampling_steps = 24
        self.unconditional_guidance_scale = 7.0
        self.denoise_strength = 0.75
    ...
    def run_image_to_image(self) -> None:
        with tqdm(self.img_data_loader) as pbar:
            for batch_idx, (input_data, filename, full_filename) in enumerate(pbar):
                ...
                if self.process_vae_in_cpu:
                    with torch.no_grad():
                        init_image = load_img(init_img_path).cpu()
                        init_image = repeat(init_image, '1 ... -> b ...', b=self.batch_size)

                        posterior = self.vae_stage.encode(init_image)
                        init_latent = posterior.sample()
                        init_latent *= 0.18215
                        init_latent = init_latent.to(self.device)
                else:
                    init_image = load_img(init_img_path).to(self.device).half()
                    init_image = repeat(init_image, '1 ... -> b ...', b=self.batch_size)
                ...
                self.sampler.make_schedule(ddim_num_steps=self.sampling_steps, ddim_eta=0.0, verbose=False)
                t_enc = int(self.denoise_strength * self.sampling_steps)
                ...
                original_image = Image.open(init_img_path).convert('RGB')
                x0 = custom_to_pil(xrec[0])
                _, h, w = xrec[0].shape
                img_out = Image.new("RGB", (2 * w, h))
                img_out.paste(original_image, (0, 0))
                img_out.paste(x0, (w, 0))
                ImageDraw.Draw(img_out).text((0, 0), f'original image', (255, 255, 255))
                ImageDraw.Draw(img_out).text((w, 0), f'{self.input_prompts}', (255, 255, 255))
                plt.imshow(img_out)
                plt.show()
...
if __name__ == '__main__':
    ...
    sd_inference.run_image_to_image()
